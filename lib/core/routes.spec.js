...
describe(specPath, () => {
	...
	describe('.constructPlugin()', () => {
		it('should load with VALID plugin architecture', () => {
			const routes = Routes(configMock)
			...
			expect(livePlugin).to.be.a('function')
		})

		it('should FAIL when passed an INVALID plugin', () => {
			const routes = Routes(configMock)
			...
			.to.throw(`Plugin ${pluginName} was not a Mink plugin`)
		})
	})
})
