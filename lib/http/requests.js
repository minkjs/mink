...
const RequestsConstructor = (config, routes) => {
	...
	requests.handleUri = (handler, fileUri, res, req) => new Promise((resolve, reject) => {
		handler.callback(fileUri, res, req).then(payload => {
	...
	requests.requestHandler = (req, res) => {
		...
		requests.resourceExists(fileUri).then(typeofResource => {
			...
			return requests.handleUri(handlerModule, matchableUri, res, req)
			.then(result => {
